<?php
  function renderForm($id, $content, $content_arabic,$photo, $error)
 {
 ?>
...
<strong>photo link:      *<br/></strong> <textarea rows="5"cols="50"type="text" name="photo" value="<?php echo $photo; ?>" >http://</textarea><br/>
 ...
 if ($content == '' || $content_arabic == '' || $photo == '')
 {
 // generate error message
 $error = 'ERROR: Please fill in all required fields!';
 
 //error, display form
 renderForm($id, $content, $content_arabic,$photo , $error);
 }
 else
 {
 // save the data to the database
 mysql_query("UPDATE newproduct SET content='$content',content_arabic='$content_arabic',photo='$photo' WHERE id='$id'")
 or die(mysql_error()); 
 
 // once saved, redirect back to the view page
 header('Location: '.$http.'e_new.php'); 
 }
 }
 else
 {
 ...
 // query db
 $id = $_GET['id'];
 $result = mysql_query("SELECT * FROM newproduct WHERE id=$id")
 or die(mysql_error()); 
 ...
 $photo = $row['photo'];
 // show form
 renderForm($id, $content, $content_arabic, $photo, '');
 }
 else
